...
const BANNED = [
  'vector', 'illustration', 'clipart', 'png', 'svg', 'digital art', 'drawing',
...
function validate(args) {
  // Support either arrays or comma-separated strings
  const register = Array.isArray(args.register) ? args.register : split(args.register);
  const bestFit = Array.isArray(args.best_fit) ? args.best_fit : split(args.best_fit);
  const proven = Array.isArray(args.proven) ? args.proven : split(args.proven);
